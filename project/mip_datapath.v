...
module mip_datapath(
    ...
    wire [4:0]  rt_mux_ex, rd_mux_ex, shamt_ex;
    wire [2:0]  aluop_ex;
    wire [5:0]  alu_control_signal;
    wire        zero_ex, alusrc_ex, regdst_ex, memtoreg_ex, memwrite_ex, memread_ex, branch_ex, bne_ex;
    ...
    wire        zero_mem, memtoreg_mem, memwrite_mem, memread_mem, branch_mem, bne_mem;

    ex_mem U_EX_MEM_REG (
        .clk(clk), .flush_exmem(Flush_exmem), .zero_ex(zero_ex), .alu_result_ex(alu_result_ex), .rd_REG_ex(rd_REG_ex),
    ...
    flush U_FLUSH_UNIT (
        ...
        .flush_ifid(Flush_ifid), .flush_idex(Flush_idex), .flush_exmem(Flush_exmem)
    );
